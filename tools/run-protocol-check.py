...
parser=argparse.ArgumentParser();parser.add_argument('parent',choices=['ftb','opac']);parser.add_argument('--loader',choices=['fabric','forge'],default='fabric');args=parser.parse_args()
...
command=[str(java),'-Xmx1536M',f'-Djava.io.tmpdir={ROOT / ".work/tmp"}']+(['-jar','fabric-server-launch.jar'] if args.loader=='fabric' else ['@libraries/net/minecraftforge/forge/1.20.1-47.4.10/win_args.txt'])+['nogui']
...
def clients():
    try:
        with (directory/'protocol-client.log').open('w') as log:
            subprocess.run(['node',str(ROOT/'tools/protocol-check.cjs'),str(port),str(result)],cwd=ROOT,stdout=log,stderr=subprocess.STDOUT,timeout=90,check=True)
