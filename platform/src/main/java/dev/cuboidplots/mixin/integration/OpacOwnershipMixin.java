...
public abstract class OpacOwnershipMixin {
    ...
    private void cuboidplots$transfer(ResourceLocation dimension,java.util.UUID owner,int subConfig,int x,int z,boolean force,org.spongepowered.asm.mixin.injection.callback.CallbackInfoReturnable<?> info) {
        var existing=((ServerClaimsManager)(Object)this).get(dimension,x,z);
        if(existing!=null&&!existing.getPlayerId().equals(owner))AddonRuntime.ownershipChanged(dimension.toString(),x,z);
    }
