...
public final class OpacClaimsAdapter implements ParentClaims {
    ...
    public OpacClaimsAdapter(MinecraftServer server) { this.server = server; }
    public String integration() { return "openpartiesandclaims"; }
    public String ownerAt(String dimension, int x, int z) {
        var claim = OpenPACServerAPI.get(server).getServerClaimsManager().get(new ResourceLocation(dimension), x, z);
        return claim == null ? null : claim.getPlayerId().toString();
    }
    public boolean canManage(UUID player, String owner) {
        // OPAC claims belong to a player, not their current party. Party access does not imply ownership.
