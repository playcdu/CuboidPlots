...
public final class FtbRanksAdapter implements LimitProvider {
  ...
  public FtbRanksAdapter(MinecraftServer server, AddonConfig config) {
    this.server = server;
    this.config = config;
  }
  ...
  public Limits effectiveLimits(UUID creator) {
    ...
    if (player == null)
      throw new IllegalStateException(
          "Creator must be online to evaluate FTB Ranks conditions for an increase");
    // Query every increase. Time, team and rank membership conditions may change at runtime.
    return new Limits(
        value(player, config.countNode, config.fallback.regions),
        value(player, config.volumeNode, config.fallback.volume));
  }
  ...
  private long value(ServerPlayer player, String node, long fallback) {
    ...
    // 1802 exposes the default sentinel through isDefaultValue, not the newer isEmpty API.
    if (permission.isDefaultValue()) return fallback;
    // asLong() can truncate numeric values. Reject fractional, overflowing or nonnumeric limits.
    Number number =
        permission
            .asNumber()
            .orElseThrow(
                () -> new IllegalArgumentException("Numeric permission required: " + node));
