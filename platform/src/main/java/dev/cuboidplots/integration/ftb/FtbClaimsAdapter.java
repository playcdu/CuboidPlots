...
public final class FtbClaimsAdapter implements ParentClaims {
  ...
  public String ownerAt(String dimension, int x, int z) {
    if (!FTBChunksAPI.api().isManagerLoaded())
      throw new IllegalStateException("FTB Chunks is not ready");
    var chunk =
        FTBChunksAPI.api()
            .getManager()
            .getChunk(
  ...
  public boolean canManage(UUID player, String owner) {
    ...
    return FTBTeamsAPI.api()
        .getManager()
        .getTeamByID(UUID.fromString(owner))
        .map(
            team ->
                team.getOwner().equals(player) || team.getRankForPlayer(player).isOfficerOrBetter())
        .orElse(false);
  }
}
