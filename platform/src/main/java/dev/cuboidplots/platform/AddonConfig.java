  ...
  public AddonConfig(Path file) throws IOException {
    ...
    if (Files.exists(file)) {
      ...
      try (Writer writer = Files.newBufferedWriter(file)) {
        values.store(
            writer,
            "CuboidPlots: -1 is explicitly unlimited; 0 allows none. Restart after editing.");
      }
    }
    fallback =
        new Limits(
            Long.parseLong(values.getProperty("default.max_regions")),
            Long.parseLong(values.getProperty("default.max_volume")));
    provider = values.getProperty("provider");
    if (!Arrays.asList("auto", "config", "ftbranks").contains(provider))
      throw new IllegalArgumentException("Unknown limit provider " + provider);
    countNode = values.getProperty("permission.max_regions");
    volumeNode = values.getProperty("permission.max_volume");
    for (String id : values.getProperty("admin_uuids").split(","))
      if (!id.trim().isEmpty()) administrators.add(UUID.fromString(id.trim()));
  }
}
