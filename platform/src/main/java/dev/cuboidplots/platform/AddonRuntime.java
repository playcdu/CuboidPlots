...
public final class AddonRuntime {
  ...
  public static void start(MinecraftServer server, Predicate<String> loaded) {
    ...
    if (ftb == opac)
      throw new IllegalStateException(
          "CuboidPlots requires exactly one parent: FTB Chunks or Open Parties and Claims. Both or"
    ...
    try {
      ...
      if (config.provider.equals("ftbranks") && !ranks)
        throw new IllegalStateException("Configured FTB Ranks provider is missing");
      ...
      RegionService service =
          new RegionService(
              parent, provider, new FileRegionStore(directory.resolve("regions.cplt")), recovery);
      recovery.resumePrepared();
      service.revalidateAll();
      regions = service;
      System.out.println(
          "[CuboidPlots] Ready: parent="
              + parentId
              + ", limits="
              + (ranks ? "ftbranks" : "config"));
    } catch (Exception ex) {
      throw new IllegalStateException("CuboidPlots initialization failed closed", ex);
    }
  }
  ...
  public static Position position(BlockPos pos) {
  ...
  public static void ownershipChanged(String dimension, int x, int z) {
    ...
    if (service != null)
      try {
        service.ownershipChanged(dimension, x, z);
      } catch (Exception ex) {
        throw new IllegalStateException(
            "CuboidPlots could not finish item recovery or save suspension. Parent ownership change"
                + " aborted.",
            ex);
      }
  }
