...
public final class Region {
  ...
  public Region(
    ...
    if (name == null || !name.matches("[a-zA-Z0-9_-]{1,48}"))
      throw new IllegalArgumentException(
          "Name must be 1-48 letters, digits, underscores or hyphens");
    this.name = name;
    this.integration = Objects.requireNonNull(integration);
    this.parentOwner = Objects.requireNonNull(parentOwner);
    this.dimension = Objects.requireNonNull(dimension);
    this.creator = Objects.requireNonNull(creator);
    this.bounds = Objects.requireNonNull(bounds);
    this.id = Objects.requireNonNull(id);
    this.assignedOwner = Objects.requireNonNull(assignedOwner);
        ...
        (player, actions) ->
            copy.put(
                ...
                Collections.unmodifiableSet(
                    actions.isEmpty() ? EnumSet.noneOf(Action.class) : EnumSet.copyOf(actions))));
    this.grants = Collections.unmodifiableMap(copy);
  }

  public Region withBounds(Cuboid replacement) {
    return new Region(
        id,
        name,
        integration,
  ...
  public Region withOwner(UUID owner) {
    return new Region(
        id, name, integration, parentOwner, dimension, creator, owner, bounds, suspended, grants);
  }

  public Region suspend() {
    return new Region(
        id,
        name,
        integration,
  ...
  public Region permission(UUID player, Action action, boolean grant) {
    ...
    if (grant) actions.add(action);
    else actions.remove(action);
    if (actions.isEmpty()) updated.remove(player);
    else updated.put(player, actions);
    return new Region(
        id,
        name,
        integration,
