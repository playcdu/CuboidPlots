...
public final class FileRegionStore implements RegionStore {
    ...
    public FileRegionStore(Path file) { this.file = file; }
    public List<Region> load() throws IOException {
        ...
        try (DataInputStream in = new DataInputStream(new ByteArrayInputStream(bytes))) {
            if (in.readInt() != MAGIC || in.readInt() != VERSION) throw new IOException("Unknown region snapshot format");
            List<Region> regions = new ArrayList<>();
            int count = count(in, 10000);
            for (int i = 0; i < count; i++) {
                UUID id = uuid(in);
                String name = in.readUTF(), integration = in.readUTF(), owner = in.readUTF(), dimension = in.readUTF();
                ...
                for (int p = 0; p < players; p++) {
                    ...
                    for (int a = 0; a < actions; a++) if (!permissions.add(Action.valueOf(in.readUTF()))) throw new IOException("Duplicate action");
                    if (grants.put(player, permissions) != null) throw new IOException("Duplicate player entry");
                }
                regions.add(new Region(id, name, integration, owner, dimension, creator, assignedOwner, bounds, suspended, grants));
            }
            if (in.readLong() != crc.getValue() || in.available() != 0) throw new IOException("Region snapshot checksum or length mismatch");
            return regions;
        } catch (RuntimeException ex) { throw new IOException("Invalid region snapshot", ex); }
    }
    public void save(List<Region> regions) throws IOException {
        ByteArrayOutputStream bytes = new ByteArrayOutputStream();
        try (DataOutputStream out = new DataOutputStream(bytes)) {
            out.writeInt(MAGIC); out.writeInt(VERSION); out.writeInt(regions.size());
            for (Region region : regions) {
                uuid(out, region.id);
                out.writeUTF(region.name); out.writeUTF(region.integration); out.writeUTF(region.parentOwner); out.writeUTF(region.dimension);
                uuid(out, region.creator); uuid(out, region.assignedOwner); position(out, region.bounds.min); position(out, region.bounds.max); out.writeBoolean(region.suspended);
                out.writeInt(region.grants.size());
                for (Map.Entry<UUID,Set<Action>> entry : region.grants.entrySet()) {
                    uuid(out, entry.getKey()); out.writeInt(entry.getValue().size());
    ...
    private static int count(DataInputStream in, int maximum) throws IOException {
        int value = in.readInt(); if (value < 0 || value > maximum) throw new IOException("Invalid record count " + value); return value;
    }
    ...
    private static Position position(DataInputStream in) throws IOException { return new Position(in.readInt(), in.readInt(), in.readInt()); }
    private static void position(DataOutputStream out, Position value) throws IOException { out.writeInt(value.x); out.writeInt(value.y); out.writeInt(value.z); }
}
