# Install the Minecraft 1.18.2 proof of concept

Install the addon **only on the server**. Use the jar for the server's loader:

- `artifacts/1.18.2/fabric/cuboidplots-1.18.2-fabric-0.1.0.jar`
- `artifacts/1.18.2/forge/cuboidplots-1.18.2-forge-0.1.0.jar`

Use Java 17. Tested loaders are Fabric Loader 0.16.14 and Forge 40.3.12. Do not install both parent claim mods; initialization explicitly rejects that configuration.

## Parent dependencies

For FTB Chunks, the exact tested releases are Chunks 1802.3.19-build.362, Teams 1802.2.11-build.152, Library 1802.3.12-build.726, Architectury 4.12.94 and Cloth Config 6.5.116. FTB Ranks 1802.1.11-build.71 is optional. Select every dependency's matching loader artifact.

For OPAC, use Open Parties and Claims 0.30.3 for Minecraft 1.18.2 and the matching loader. On Fabric also install Fabric API 0.77.0+1.18.2 and Forge Config API Port 3.2.4. The dedicated tests also had Architectury and Cloth Config installed.

Fabric API 0.77.0+1.18.2 is required by the Fabric addon. The addon bundles its own portable core and MixinExtras; no separate installation of those is needed. Exact dependency filenames, official download links and checksums are recorded in `reports/1.18.2/<loader>/<parent>/dependencies.json`. Third-party mod jars are not redistributed in the artifacts directory.
